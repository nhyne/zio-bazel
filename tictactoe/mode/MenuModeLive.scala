...
import dev.nhyne.tictactoe.parser.MenuCommandParser
import dev.nhyne.tictactoe.view.MenuView
import zio.UIO
    ...
    def process(input: String, state: State.Menu): UIO[State] =
      menuCommandParser.parse(input) map {
        ...
        case MenuCommand.Invalid =>
          state.copy(message = MenuMessage.InvalidCommand)
      }
