...
import dev.nhyne.tictactoe.logic.{GameLogic, OpponentAi}
import dev.nhyne.tictactoe.parser.GameCommandParser
import dev.nhyne.tictactoe.view.GameView
import zio.UIO
  ...
  val gameMode = new GameMode.Service[Any] {
    ...
    def process(input: String, state: State.Game): UIO[State] =
      if (state.result != GameResult.Ongoing)
        UIO.succeed(State.Menu(None, MenuMessage.Empty))
    ...
    def render(state: State.Game): UIO[String] = {
      val player = if (state.turn == Piece.X) state.cross else state.nought
      for {
